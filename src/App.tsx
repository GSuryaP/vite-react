...
const SKILL_CATEGORIES = [
  {
    ...
    skills: [
      { name: "Python", pct: 90 }, { name: "JavaScript", pct: 85 },
      { name: "C++", pct: 77 }, { name: "C", pct: 74 },
    ],
  },
  {
    label: "Frontend", icon: <Layers size={18} />, sub: "Interface layer",
    skills: [
      { name: "React", pct: 82 }, { name: "HTML5", pct: 92 },
      { name: "CSS3", pct: 88 }, { name: "Tailwind CSS", pct: 86 },
    ],
  },
  {
    label: "Backend & Data", icon: <Database size={18} />, sub: "Server & storage",
    skills: [
      { name: "Node.js", pct: 80 }, { name: "Express", pct: 78 },
      { name: "MongoDB", pct: 75 }, { name: "MySQL", pct: 73 }, { name: "Firebase", pct: 70 },
    ],
  },
  {
    label: "Tooling", icon: <Wrench size={18} />, sub: "Ground support",
    skills: [{ name: "Git", pct: 84 }, { name: "GitHub", pct: 83 }],
  },
];
  ...
  <Reveal delay={index * 0.09} dir={index % 2 ? "right" : "left"} style={{ height: "100%" }}>
    <div style={{ height: "100%", border: `1px solid ${T.line}`, background: T.bgPanel, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
        {cat.skills.map((s, i) => <SkillBar key={s.name} skill={s} delay={i * 0.05} />)}
      </div>
    </div>
  </Reveal>
);
...
// /* ============================================================
//    CINEMATIC SCROLL REVEAL — hard-edge wipe
//    No fade, no blur, no rise-and-settle. Each block is masked by a
//    real CSS clip-path that a scroll-driven progress value retracts,
//    like a shutter opening over the content. Vertical blocks wipe
//    open top-down; "back" blocks wipe open left-to-right instead, so
//    the page alternates axis rather than alternating tilt direction.
//    A thin accent line rides the retracting edge and vanishes once
//    the block is fully open. Progress is still lerped for smoothness
//    and is fully reversible with scroll position, but the content
//    itself never fades or blurs — it's simply uncovered.
// ============================================================ */
// const REVEAL_START = 0.9;   // fraction of viewport height: progress = 0
// const REVEAL_END = 0.5;     // fraction of viewport height: progress = 1

// let revealItems = [];

// function stepReveal() {
//   const vh = window.innerHeight;
//   const startY = vh * REVEAL_START;
//   const endY = vh * REVEAL_END;

//   for (const item of revealItems) {
//     const el = item.box;
//     if (!el) continue;
//     const rect = el.getBoundingClientRect();
//     let target = (startY - rect.top) / (startY - endY);
//     target = target < 0 ? 0 : target > 1 ? 1 : target;
//     item.cur = item.cur === undefined ? target : item.cur + (target - item.cur) * 0.16;
//     const p = item.cur;
//     const eased = p < 0.5 ? 4 * p * p * p : 1 - Math.pow(-2 * p + 2, 3) / 2;
//     const closed = ((1 - eased) * 100).toFixed(2);

//     if (item.axis === "x") {
//       el.style.clipPath = `inset(0 ${closed}% 0 0)`;
//     } else {
//       el.style.clipPath = `inset(0 0 ${closed}% 0)`;
//     }
//     const edge = item.edge;
//     if (edge) {
//       const atEdge = eased > 0.015 && eased < 0.985;
//       edge.style.opacity = atEdge ? "1" : "0";
//       if (item.axis === "x") {
//         edge.style.left = `calc(${(eased * 100).toFixed(2)}% - 1px)`;
//       } else {
//         edge.style.top = `calc(${(eased * 100).toFixed(2)}% - 1px)`;
//       }
//     }
//   }
// }
...
//   useEffect(() => {
//     let raf;
//     const loop = () => { stepReveal(); raf = requestAnimationFrame(loop); };
//     raf = requestAnimationFrame(loop);
//     return () => cancelAnimationFrame(raf);
//   }, []);
// }

// const Reveal = ({ children, back = false, className, style }) => {
//   const boxRef = useRef(null);
//   const edgeRef = useRef(null);
//   const axis = back ? "x" : "y";
//   useEffect(() => {
//     const item = { box: boxRef.current, edge: edgeRef.current, axis, cur: undefined };
//     revealItems.push(item);
//     return () => { revealItems = revealItems.filter((i) => i !== item); };
//   }, [axis]);
//   return (
//     <div
//       ref={boxRef}
...
//                 background: `linear-gradient(180deg, transparent, ${T.accent2}, transparent)`,
//                 boxShadow: `0 0 14px ${T.accent2}aa`, opacity: 0, pointerEvents: "none",
//               }
//             : {
//                 position: "absolute", left: 0, right: 0, top: 0, height: 2,
//                 background: `linear-gradient(90deg, transparent, ${T.accent2}, transparent)`,
//                 boxShadow: `0 0 14px ${T.accent2}aa`, opacity: 0, pointerEvents: "none",
//               }
//         }
//       />
...
// /* Scroll "warp" — the whole page tilts straight back in 3D space
//    (rotateX only, no skew/shear) and zoom-pulses with velocity, then
//    springs flat. Reads as the camera getting shoved, not a slide. */
// function useScrollWarp(ref) {
//   useEffect(() => {
//     let lastY = window.scrollY;
//     let raf = null;
//     let tilt = 0;
...
//       const delta = y - lastY;
//       lastY = y;
//       const targetTilt = Math.max(-10, Math.min(10, delta * 0.7));
//       const targetZoom = Math.max(0, Math.min(1, Math.abs(delta) * 0.06));
//       tilt += (targetTilt - tilt) * 0.22;
//       zoom += (targetZoom - zoom) * 0.18;
//       if (ref.current) {
//         const scale = 1 - zoom * 0.035;
//         ref.current.style.transform = `perspective(1600px) rotateX(${tilt.toFixed(3)}deg) scale(${scale.toFixed(4)})`;
//       }
//       if (Math.abs(tilt) > 0.02 || zoom > 0.005) {
//         raf = requestAnimationFrame(tick);
//       } else {
//         raf = null;
//       }
...
// const SkillPanel = ({ cat, index }) => (
//   <Reveal back={index % 2 === 1} style={{ height: "100%" }}>
//     <div style={{ height: "100%", border: `1px solid ${T.line}`, background: T.bgPanel, display: "flex", flexDirection: "column" }}>
//       <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 18px", borderBottom: `1px solid ${T.line}`, background: T.bgAlt }}>
//         <div style={{ width: 36, height: 36, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", color: T.accent2, background: T.accentSoft, border: `1px solid ${T.accent}44` }}>{cat.icon}</div>
//         <div>
...
//       ::-webkit-scrollbar-thumb{background:${T.accent}}
//       @keyframes slide-up{from{opacity:0;transform:translate3d(0,50px,0) scale(.9)}to{opacity:1;transform:none}}
//       @keyframes portrait-flip{from{opacity:0;transform:scale(.7) rotateX(60deg);filter:blur(8px)}to{opacity:1;transform:none;filter:blur(0)}}
//       @keyframes pulse-glow{0%,100%{opacity:.5}50%{opacity:1}}
//       @keyframes drift{0%{transform:translate(0,0)}50%{transform:translate(-30px,22px)}100%{transform:translate(0,0)}}
//       @media(max-width:900px){.warp-wrap{transform:none!important}}
//       @media(max-width:768px){.nav-desktop{display:none!important}.nav-ham{display:flex!important}.hero-grid{grid-template-columns:1fr!important}.hero-grid > div:last-child{order:-1;justify-content:center;display:flex}.about-grid{grid-template-columns:1fr!important}.contact-grid{grid-template-columns:1fr!important}}
//     `;
//     const old = document.getElementById("portfolio-base");
//     if (old) document.head.removeChild(old);
...
//       <div ref={warpRef} className="warp-wrap" style={{ transformOrigin: "50% 50%", transition: "transform .05s linear" }}>
...
//               <div style={{ animation: "portrait-flip .8s .18s cubic-bezier(.19,1.4,.36,1) forwards", opacity: 0, display: "flex", justifyContent: "center", perspective: 1200 }}>
//                 <Portrait />
//               </div>
//             </div>
//           </div>
