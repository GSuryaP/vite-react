...
import { Code2, Layers, Database, Wrench, GitBranch, CircleDot, Bug, TriangleAlert } from "lucide-react";
...
/* ============================================================
   DESIGN DIRECTION — "// COMPILE.TSX"
   A real VS-Code-dark-theme-native aesthetic: a faux IDE window
   chrome (traffic lights, file tabs per section, line-number
   gutters, a status bar) where the whole portfolio reads like
   an open editor. Copy is reframed as code/comments/console
   output. Palette is lifted straight from One-Dark/VS-Code
   syntax tokens — purple keywords, green strings, yellow
   functions, orange constants, blue accents — so every label
   and value looks like it's actually being syntax-highlighted.
   Signature element: the "STATUS BAR" footer-rail (git branch,
   0 errors / 0 warnings, line:col, UTF-8, the works) plus a
   live blinking terminal caret driving the hero boot text.
   Fonts: JetBrains Mono carries ~everything (it IS a code
   editor); Space Grotesk only for the giant name so it still
   reads as a human, not a robot.
============================================================ */
interface Theme {
  bg: string; bgAlt: string; bgCard: string; bgPanel: string; bgGutter: string;
  ink: string;
  kw: string;      // keyword / purple — primary "signal" accent
  str: string;     // string / green
  fn: string;      // function / yellow
  num: string;     // number-constant / orange
  blue: string;    // accent blue / clock
...
const T: Theme = {
  bg: "#0d1117", bgAlt: "#0a0e14", bgCard: "#161b22", bgPanel: "#010409", bgGutter: "#0a0e14",
  ink: "#c9d1d9",
  kw: "#c586c0", str: "#7ee787", fn: "#e3b341", num: "#f0883e",
  blue: "#58a6ff", red: "#f85149",
  line: "rgba(201,217,217,0.09)", lineStrong: "rgba(201,217,217,0.22)",
  textMuted: "#8b949e", textFaint: "#484f58",
  navBg: "rgba(10,14,20,0.92)",
};
// aliases so the rest of the file (lifted from the prior layout) maps cleanly
const signal = T.str;      // headline / glow accent ("string green")
const signalSoft = "rgba(126,231,135,0.12)";
const clock = T.blue;      // secondary accent ("blue")
const clockSoft = "rgba(88,166,255,0.13)";
...
/* ============================================================
   LINE-NUMBER GUTTER DIVIDER — signature transition element.
   Reads exactly like the left gutter of a code editor scrolling
   past a closing brace + blank lines between two "files".
============================================================ */
let GUTTER_LINE = 1;
const GutterDivider: React.FC<{ count?: number; closeBrace?: boolean }> = ({ count = 3, closeBrace = true }) => {
  const start = (GUTTER_LINE += count + (closeBrace ? 1 : 0)) - count - (closeBrace ? 1 : 0);
  const rows = Array.from({ length: count });
  return (
    <div style={{ background: T.bgGutter, borderTop: `1px solid ${T.line}`, borderBottom: `1px solid ${T.line}`, position: "relative", zIndex: 2 }}>
      {closeBrace && (
        <div style={{ display: "flex", fontFamily: "'JetBrains Mono',monospace", fontSize: 12 }}>
          <span style={{ width: 56, textAlign: "right", paddingRight: 14, color: T.textFaint, userSelect: "none", flexShrink: 0 }}>{start}</span>
          <span style={{ color: T.ink, padding: "2px 0" }}>{"}"}</span>
        </div>
      )}
      {rows.map((_, i) => (
        <div key={i} style={{ display: "flex", fontFamily: "'JetBrains Mono',monospace", fontSize: 12 }}>
          <span style={{ width: 56, textAlign: "right", paddingRight: 14, color: T.textFaint, userSelect: "none", flexShrink: 0 }}>{start + (closeBrace ? 1 : 0) + i}</span>
          <span>&nbsp;</span>
        </div>
      ))}
    </div>
...
/* ============================================================
   FIELD — labeled syntax-highlighted value: key: "value"
============================================================ */
const Field: React.FC<{ label: string; children: React.ReactNode; accent?: boolean }> = ({ label, children, accent }) => (
  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12.5 }}>
    <span style={{ color: T.blue }}>{label}</span>
    <span style={{ color: T.textFaint }}>: </span>
    <span style={{ color: accent ? signal : T.fn, fontWeight: 600 }}>"{children}"</span>
  </div>
);

/* ============================================================
   ANIMATED COUNTER — rendered as a const declaration
============================================================ */
const Counter: React.FC<{ to: string; suffix?: string; label: string }> = ({ to, suffix = "", label }) => {
  const [val, setVal] = useState("0");
  const { ref, visible } = useReveal();
  ...
  return (
    <div ref={ref} style={{ textAlign: "left", padding: "20px 18px", background: T.bgCard, border: `1px solid ${T.line}`, position: "relative", fontFamily: "'JetBrains Mono',monospace" }}>
      <div style={{ fontSize: 10, color: T.textFaint, marginBottom: 8 }}>
        <span style={{ color: T.kw }}>const</span> <span style={{ color: T.blue }}>{label.toLowerCase().replace(/\s+/g, "_")}</span> =
      </div>
      <div style={{ fontSize: "clamp(24px,3.2vw,32px)", fontWeight: 700, lineHeight: 1, color: T.ink }}>
        {val}<span style={{ fontSize: "0.6em", color: signal }}>{suffix}</span><span style={{ color: T.textFaint, fontSize: "0.45em" }}>;</span>
      </div>
    </div>
  );
};
...
/* ============================================================
   SKILL ROW — looks like an import line
============================================================ */
const SkillCard: React.FC<{ skill: Skill; delay: number }> = ({ skill, delay }) => {
  const { ref, visible } = useReveal();
  const [hov, setHov] = useState(false);
  return (
    <div
      ...
      style={{
        opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(10px)",
        transition: `opacity .5s ease ${delay}s, transform .5s ease ${delay}s, border-color .2s, background .2s, box-shadow .2s`,
        background: hov ? signalSoft : T.bgCard,
        border: `1px solid ${hov ? signal : T.line}`,
        boxShadow: hov ? `0 0 16px -6px ${signal}99` : "none",
        padding: "10px 12px", display: "flex", alignItems: "center", gap: 10,
      }}>
      <div style={{ fontSize: 16, color: skill.color, flexShrink: 0, width: 20, display: "flex", justifyContent: "center" }}>{skill.icon}</div>
      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12 }}>
        <span style={{ color: T.kw }}>import</span>{" "}
        <span style={{ color: hov ? signal : T.ink, fontWeight: 600 }}>{skill.name.replace(/[^A-Za-z0-9]/g, "")}</span>
      </span>
    </div>
  );
};
...
/* ============================================================
   SKILL CATEGORY PANEL — modeled as a code file in the editor
============================================================ */
const SkillCategoryPanel: React.FC<{ cat: SkillCategory; index: number }> = ({ cat, index }) => (
  <Reveal delay={index * 0.08} style={{ height: "100%" }}>
    <div style={{ height: "100%", border: `1px solid ${T.line}`, background: T.bgPanel, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderBottom: `1px solid ${T.line}`, background: T.bgAlt }}>
        <div style={{ width: 30, height: 30, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", color: signal, background: signalSoft, border: `1px solid ${signal}44` }}>
          {cat.icon}
        </div>
        <div style={{ fontFamily: "'JetBrains Mono',monospace" }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: T.ink }}>
            {cat.label.toLowerCase().replace(/\s*&\s*|\s+/g, "_")}<span style={{ color: T.textFaint }}>.module.ts</span>
          </div>
          <div style={{ fontSize: 9.5, color: T.textFaint, letterSpacing: "0.06em" }}>
            {cat.skills.length} export{cat.skills.length !== 1 ? "s" : ""}
          </div>
        </div>
      </div>
...
/* ============================================================
   EXPERIENCE CARD — collapsible like a folded code block
============================================================ */
const ExpCard: React.FC<{ exp: Experience; index: number }> = ({ exp, index }) => {
  const [open, setOpen] = useState(index === 0);
  return (
    <Reveal delay={index * 0.08}>
      <div style={{ border: `1px solid ${T.line}`, background: T.bgCard }}>
        <button
          onClick={() => setOpen(o => !o)}
          style={{ width: "100%", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, padding: "18px 20px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flex: 1, minWidth: 0 }}>
            <span style={{ color: T.textFaint, fontFamily: "'JetBrains Mono',monospace", fontSize: 13, marginTop: 2, flexShrink: 0 }}>{open ? "▾" : "▸"}</span>
            <div style={{ minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 5, fontFamily: "'JetBrains Mono',monospace" }}>
                <span style={{ fontSize: 14.5, fontWeight: 700, color: T.ink }}>
                  <span style={{ color: T.kw }}>function</span> {exp.role.split(" ").slice(0,3).join("")}<span style={{color: T.textFaint}}>()</span>
                </span>
                <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 8px", background: signalSoft, color: signal, border: `1px solid ${signal}55`, letterSpacing: "0.1em", textTransform: "uppercase" }}>{exp.badge}</span>
              </div>
              <div style={{ fontSize: 13, fontWeight: 600, color: T.textMuted, marginBottom: 2, fontFamily: "'JetBrains Mono',monospace" }}>// {exp.role}</div>
              <div style={{ fontSize: 12.5, color: T.fn, marginBottom: 2 }}>{exp.org}</div>
              <div style={{ fontSize: 11, color: T.textFaint, fontFamily: "'JetBrains Mono',monospace" }}>{exp.period}</div>
            </div>
          </div>
        </button>
        {open && (
          <div style={{ padding: "0 20px 20px", borderTop: `1px solid ${T.line}` }}>
            <div style={{ paddingTop: 14, marginLeft: 26, borderLeft: `1px solid ${T.line}`, paddingLeft: 18 }}>
              {exp.bullets.map((b, i) => (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                  <span style={{ color: T.blue, flexShrink: 0, fontFamily: "'JetBrains Mono',monospace", fontSize: 12, marginTop: 2 }}>{i+1}.</span>
                  <p style={{ fontSize: 13, color: T.textMuted, lineHeight: 1.75 }}>{b}</p>
                </div>
              ))}
              {exp.tags.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 14, paddingTop: 14, borderTop: `1px solid ${T.line}`, fontFamily: "'JetBrains Mono',monospace" }}>
                  <span style={{ fontSize: 10, color: T.textFaint, marginRight: 2 }}>stack:</span>
                  {exp.tags.map(t => (
                    <span key={t} style={{ fontSize: 10, fontWeight: 600, padding: "3px 9px", background: T.bgAlt, border: `1px solid ${T.line}`, color: T.num }}>{t}</span>
                  ))}
                </div>
              )}
            </div>
...
/* ============================================================
   PROJECT CARD — terminal/repo card
============================================================ */
const ProjCard: React.FC<{ proj: Project; index: number }> = ({ proj, index }) => {
  const [hov, setHov] = useState(false);
  return (
    <Reveal delay={index * 0.06} style={{ height: "100%" }}>
      <div
        ...
        style={{
          ...
          background: T.bgCard, transition: "border-color .2s, transform .2s, box-shadow .2s",
          transform: hov ? "translateY(-3px)" : "none",
          boxShadow: hov ? `0 14px 30px -12px ${signal}55` : "none",
        }}>
        <div style={{ padding: "9px 14px", display: "flex", alignItems: "center", gap: 8, borderBottom: `1px solid ${T.line}`, background: T.bgAlt, fontFamily: "'JetBrains Mono',monospace" }}>
          <SiGithub size={11} color={T.textFaint} />
          <span style={{ fontSize: 10.5, color: T.textMuted }}>
            GSuryaP<span style={{ color: T.textFaint }}>/</span><span style={{ color: signal }}>{proj.title.split(" ").slice(0,2).join("-")}</span>
          </span>
          {proj.repo && (
            <a href={proj.repo} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}
              style={{ marginLeft: "auto", color: hov ? signal : T.textFaint, display: "flex", alignItems: "center", gap: 4, fontSize: 9.5, transition: "color .2s" }}>
              open ↗
            </a>
          )}
        </div>
        <div style={{ padding: "16px 18px 20px", flex: 1, display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
            ...
            <h3 style={{ fontSize: 14.5, fontWeight: 700, color: T.ink, fontFamily: "'Space Grotesk',sans-serif", lineHeight: 1.3 }}>{proj.title}</h3>
          </div>
          <p style={{ fontSize: 12.5, color: T.textMuted, lineHeight: 1.75, flex: 1, marginBottom: 16 }}>{proj.desc}</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, fontFamily: "'JetBrains Mono',monospace" }}>
            {proj.tags.map(t => (
              <span key={t} style={{ fontSize: 10, fontWeight: 600, padding: "3px 9px", background: T.bgAlt, border: `1px solid ${T.line}`, color: T.fn }}>{t}</span>
            ))}
          </div>
        </div>
      </div>
...
/* ============================================================
   SECTION TITLE — rendered as a code comment block
============================================================ */
const STitle: React.FC<{ tag: string; title: string; sub?: string }> = ({ tag, title, sub }) => (
  <Reveal style={{ marginBottom: 44 }}>
    <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, color: T.textFaint, marginBottom: 12 }}>
      <span style={{ color: T.textFaint }}>{"/* "}</span>
      <span style={{ color: signal }}>{tag}</span>
      <span style={{ color: T.textFaint }}>{" */"}</span>
    </div>
    <h2 style={{ fontSize: "clamp(26px,3.8vw,40px)", fontWeight: 700, color: T.ink, lineHeight: 1.15, letterSpacing: "-0.01em", fontFamily: "'Space Grotesk',sans-serif" }}>
      <span style={{ color: T.kw }}>class</span> {title.replace(/\s+/g, "")} <span style={{ color: T.textFaint }}>{"{"}</span>
    </h2>
    {sub && <p style={{ fontSize: 13.5, color: T.textMuted, marginTop: 12, maxWidth: 540, lineHeight: 1.75 }}>{sub}</p>}
  </Reveal>
);

/* ============================================================
   NAV LINK — rendered like an editor tab
============================================================ */
const NavLink: React.FC<{ href: string; label: string; active: boolean }> = ({ href, label, active }) => (
  <a href={href} style={{
    fontSize: 11, fontWeight: 600, padding: "9px 14px",
    color: active ? T.ink : T.textMuted,
    background: active ? T.bgCard : "transparent",
    borderTop: active ? `2px solid ${signal}` : "2px solid transparent",
    borderRight: `1px solid ${T.line}`,
    fontFamily: "'JetBrains Mono',monospace", textDecoration: "none",
    transition: "color .2s, background .2s",
    display: "flex", alignItems: "center", gap: 6,
  }}>
    <span style={{ width: 6, height: 6, borderRadius: "50%", background: active ? signal : T.textFaint, flexShrink: 0 }} />
    {label.toLowerCase()}.tsx
  </a>
);
...
const ContactRow: React.FC<{ icon: string; label: string; val: string; href?: string; isAccent?: boolean }> = ({ icon, label, val, href, isAccent }) => {
  ...
  return (
    <Tag {...linkProps} style={{
      ...
      <div style={{ width: 32, height: 32, flexShrink: 0, background: signalSoft, border: `1px solid ${signal}33`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: signal }}>{icon}</div>
      <div style={{ fontFamily: "'JetBrains Mono',monospace" }}>
        <div style={{ fontSize: 9, color: T.blue, letterSpacing: "0.06em", marginBottom: 2 }}>{label.toLowerCase()}:</div>
        <div style={{ fontSize: 12.5, fontWeight: 600, color: isAccent ? signal : T.ink }}>"{val}"</div>
      </div>
    </Tag>
  );
};
  ...
  return (
    <a
      ...
      style={{
        display: "flex", alignItems: "center", justifyContent: "center", gap: 7,
        padding: "11px", fontWeight: 700, fontSize: 11,
        border: `1px solid ${hov ? signal : T.line}`, background: T.bgCard, color: hov ? signal : T.ink,
        fontFamily: "'JetBrains Mono',monospace", letterSpacing: "0.04em",
        textTransform: "none", textDecoration: "none", transition: "border-color .2s, color .2s",
      }}>
      {icon} {label}
    </a>
  );
...
const HeroBtn: React.FC<{ href: string; primary?: boolean; children: React.ReactNode }> = ({ href, primary, children }) => {
  ...
  return (
    <a href={href} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      ...
      textDecoration: "none",
      ...(primary
        ? { background: signal, color: "#03130a", boxShadow: hov ? `0 0 26px ${signal}88` : `0 0 0px ${signal}00` }
        : { border: `1px solid ${hov ? clock : T.lineStrong}`, background: "transparent", color: hov ? clock : T.ink }),
...
const WABtn: React.FC = () => (
  <a
    ...
    style={{
      display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
      background: `linear-gradient(90deg,${signal},${clock})`, color: "#03050a", fontWeight: 700, fontSize: 12,
      padding: "13px 20px", textDecoration: "none", fontFamily: "'JetBrains Mono',monospace",
    }}>
...
const GithubLink: React.FC = () => (
  <a href={DATA.github} target="_blank" rel="noreferrer" style={{
    ...
    <SiGithub size={13} /> $ git clone github.com/GSuryaP ↗
  </a>
);
...
const BackToTop: React.FC<{ show: boolean }> = ({ show }) => (
  <button
    onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
    style={{
      position: "fixed", bottom: 56, right: 24, zIndex: 50,
      width: 38, height: 38, background: T.bgPanel,
      color: signal, border: `1px solid ${T.lineStrong}`, fontSize: 14,
      display: "flex", alignItems: "center", justifyContent: "center",
      opacity: show ? 1 : 0, pointerEvents: show ? "all" : "none",
      transform: show ? "translateY(0)" : "translateY(10px)",
      boxShadow: show ? `0 6px 22px ${signal}33` : "none",
      transition: "opacity .3s, transform .3s, box-shadow .3s",
      cursor: "pointer", fontFamily: "'JetBrains Mono',monospace",
    }}>↑</button>
);
...
/* ============================================================
   STATUS BAR — signature element: a live VS-Code-style footer
   rail that pins the whole page to the "editor" conceit.
============================================================ */
const StatusBar: React.FC<{ section: string }> = ({ section }) => (
  <div style={{
    position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 60,
    background: clock, color: "#04101f",
    fontFamily: "'JetBrains Mono',monospace", fontSize: 11, fontWeight: 600,
    display: "flex", alignItems: "center", justifyContent: "space-between",
    padding: "4px 16px", letterSpacing: "0.01em",
  }}>
    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
      <span style={{ display: "flex", alignItems: "center", gap: 5 }}><GitBranch size={12} /> main</span>
      <span style={{ display: "flex", alignItems: "center", gap: 5 }}><Bug size={12} /> 0</span>
      <span style={{ display: "flex", alignItems: "center", gap: 5 }}><TriangleAlert size={12} /> 0</span>
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
      <span className="status-section-label">{section.toLowerCase()}.tsx</span>
      <span>UTF-8</span>
      <span>TSX</span>
      <span style={{ display: "flex", alignItems: "center", gap: 5 }}><CircleDot size={11} /> open to work</span>
    </div>
  </div>
);
...
export default function App() {
  ...
  useEffect(() => {
    const seq = [
      ...
      "compiling surya.dev ...",
      "✓ resolved 6 projects",
      "✓ resolved 1 internship",
      "✓ 0 errors, 0 warnings",
    ];
    let li = 0, ci = 0;
    const iv = setInterval(() => {
      if (li >= seq.length) { clearInterval(iv); setBootDone(true); return; }
      const cur = seq[li];
      if (ci <= cur.length) {
        setTypedText(cur.slice(0, ci));
        ci++;
      } else {
        setBootLines(prev => [...prev, cur]);
        li++; ci = 0;
        setTypedText("");
      }
    }, 26);
  ...
  useEffect(() => {
    ...
    s.textContent = `
      ...
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      ::-webkit-scrollbar{width:4px}
      ::-webkit-scrollbar-track{background:${T.bg}}
      ::-webkit-scrollbar-thumb{background:${signal}}
      @keyframes slide-up{from{opacity:0;transform:translateY(22px)}to{opacity:1;transform:none}}
      @keyframes slide-right{from{opacity:0;transform:translateX(-22px)}to{opacity:1;transform:none}}
      @keyframes blink{0%,100%{opacity:1}50%{opacity:0}}
      @keyframes pulse-glow{0%,100%{opacity:.55}50%{opacity:1}}
      @keyframes scan{0%{background-position:0 0}100%{background-position:0 6px}}
      @media(max-width:768px){.nav-desktop{display:none!important}.nav-ham{display:flex!important}.hero-grid{grid-template-columns:1fr!important}.about-grid{grid-template-columns:1fr!important}.contact-grid{grid-template-columns:1fr!important}.status-section-label{display:none!important}}
    `;
    const old = document.getElementById("portfolio-base");
    if (old) document.head.removeChild(old);
    document.head.appendChild(s);
  ...
  const C: React.CSSProperties = { maxWidth: 1140, margin: "0 auto", padding: "0 24px" };
  const SP: React.CSSProperties = { position: "relative", zIndex: 2, padding: "90px 0" };

  return (
    <div style={{ background: T.bg, color: T.ink, minHeight: "100vh", paddingBottom: 26 }}>

      {/* faint scanline + dot-grid editor backdrop */}
      <div style={{
      ...
      <div style={{
        position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", opacity: 0.5,
        backgroundImage: `
          radial-gradient(circle at 12% 16%, ${signalSoft} 0%, transparent 32%),
          radial-gradient(circle at 88% 80%, ${clockSoft} 0%, transparent 32%)`,
        animation: "pulse-glow 7s ease-in-out infinite",
      }} />
      <div style={{
      ...
      {/* ── NAV — editor tab bar ── */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, transition: "all .3s", background: T.navBg, backdropFilter: "blur(14px)", borderBottom: `1px solid ${T.line}` }}>
        {/* titlebar */}
        <div style={{ padding: "8px 0", borderBottom: `1px solid ${T.line}` }}>
          <div style={{ ...C, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <a href="#home" style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 16, color: T.ink, display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ display: "flex", gap: 5 }}>
                <span style={{ width: 9, height: 9, borderRadius: "50%", background: T.red, display: "inline-block" }} />
                <span style={{ width: 9, height: 9, borderRadius: "50%", background: T.num, display: "inline-block" }} />
                <span style={{ width: 9, height: 9, borderRadius: "50%", background: signal, display: "inline-block" }} />
              </span>
              <span style={{ color: T.textMuted, fontFamily: "'JetBrains Mono',monospace", fontSize: 12 }}>surya@portfolio:~$</span>
            </a>
            <button className="nav-ham" onClick={() => setMobile(o => !o)}
              style={{ display: "none", flexDirection: "column", gap: 4, background: "none", border: `1px solid ${T.line}`, padding: "7px 9px" }}>
              {[0,1,2].map(i => <span key={i} style={{ width: 18, height: 1, background: T.ink, display: "block" }} />)}
        ...
        {/* tabs */}
        <div className="nav-desktop" style={{ ...C, display: "flex", padding: 0 }}>
          {NAV_ITEMS.map(n => <NavLink key={n} href={`#${n.toLowerCase()}`} label={n} active={active === n} />)}
        </div>
        {mobileOpen && (
          <div style={{ background: T.navBg, backdropFilter: "blur(14px)", borderTop: `1px solid ${T.line}`, padding: "10px 16px" }}>
            {NAV_ITEMS.map(n => (
              <a key={n} href={`#${n.toLowerCase()}`} onClick={() => setMobile(false)}
                style={{ display: "block", padding: "12px 14px", fontSize: 12, fontWeight: 600, color: active === n ? signal : T.textMuted, borderBottom: `1px solid ${T.line}`, fontFamily: "'JetBrains Mono',monospace" }}>
                {n.toLowerCase()}.tsx
              </a>
            ))}
          </div>
      ...
      {/* ── HERO ── */}
      <section id="home" style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "150px 0 70px", position: "relative", zIndex: 2 }}>
        <div style={{ ...C, position: "relative", zIndex: 1 }}>
          <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 60, alignItems: "center" }}>
            <div>
              <div style={{ animation: "slide-up .55s ease forwards", opacity: 0, animationDelay: "0.05s" }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, border: `1px solid ${signal}55`, background: signalSoft, padding: "5px 14px", marginBottom: 22, fontFamily: "'JetBrains Mono',monospace" }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: signal, animation: "pulse-glow 1.4s ease-in-out infinite" }} />
                  <span style={{ fontSize: 10, fontWeight: 700, color: signal }}>STATUS · AVAILABLE_FOR_HIRE = true</span>
                </div>
              </div>
              <div style={{ animation: "slide-up .55s ease forwards", opacity: 0, animationDelay: "0.12s" }}>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12.5, color: T.textFaint, marginBottom: 10 }}>
                  <span style={{ color: T.kw }}>const</span> <span style={{ color: T.blue }}>developer</span> <span style={{ color: T.textFaint }}>=</span> <span style={{ color: T.textFaint }}>{"{"}</span>
                </div>
                <h1 style={{ fontSize: "clamp(32px,4.8vw,56px)", fontWeight: 700, lineHeight: 1.08, letterSpacing: "-0.01em", marginBottom: 6, fontFamily: "'Space Grotesk',sans-serif" }}>
                  <span style={{ color: T.ink }}>{DATA.name.split("\n")[0]}</span><br />
                  <span style={{ color: signal }}>{DATA.name.split("\n")[1]}</span>
                </h1>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12.5, color: T.textFaint, marginBottom: 18 }}>{"};"}</div>
              </div>
              <div style={{ animation: "slide-up .55s ease forwards", opacity: 0, animationDelay: "0.2s" }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: signal, marginBottom: 6, fontFamily: "'JetBrains Mono',monospace", minHeight: 18 }}>
                  {bootLines.map((l, i) => (
                    <div key={i} style={{ color: l.startsWith("✓") ? T.fn : T.textMuted }}>{l}</div>
                  ))}
                  <div>{typedText}<span style={{ animation: "blink 1s step-end infinite" }}>▌</span></div>
                </div>
                <div style={{ fontSize: 14, fontWeight: 600, color: T.textMuted, margin: "8px 0", fontFamily: "'JetBrains Mono',monospace", opacity: bootDone ? 1 : 0, transition: "opacity .4s" }}>
                  // {DATA.role}
                </div>
                <p style={{ fontSize: 14, color: T.textMuted, lineHeight: 1.75, maxWidth: 480, marginBottom: 30 }}>{DATA.tagline}</p>
              </div>
              <div style={{ animation: "slide-up .55s ease forwards", opacity: 0, animationDelay: "0.28s", display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 40 }}>
                <HeroBtn href="#contact" primary>$ contact --me</HeroBtn>
                <HeroBtn href="#projects">$ ls ./projects</HeroBtn>
              </div>
              <div style={{ animation: "slide-up .55s ease forwards", opacity: 0, animationDelay: "0.36s", display: "grid", gridTemplateColumns: "repeat(4,auto)", gap: 32, paddingTop: 22, borderTop: `1px solid ${T.line}`, flexWrap: "wrap" }}>
                {[["8.73","CGPA"],["6+","PROJECTS"],["1","INTERNSHIP"],["2+","CLUB ROLES"]].map(([v,l]) => (
                  <div key={l} style={{ fontFamily: "'JetBrains Mono',monospace" }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: T.ink, fontFamily: "'Space Grotesk',sans-serif" }}>{v}</div>
                    <div style={{ fontSize: 9, color: T.textFaint, marginTop: 3 }}>// {l.toLowerCase()}</div>
                  </div>
                ))}
              </div>
            </div>

            {/* Signature: editor-window profile card — image shown in full */}
            <div style={{ animation: "slide-right .6s .15s ease forwards", opacity: 0, width: 300 }}>
              <div style={{ border: `1px solid ${T.lineStrong}`, background: T.bgPanel, boxShadow: `0 10px 40px rgba(0,0,0,0.55), 0 0 0 1px ${signal}11` }}>
                <div style={{ borderBottom: `1px solid ${T.line}`, padding: "9px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", background: T.bgAlt, fontFamily: "'JetBrains Mono',monospace" }}>
                  <span style={{ fontSize: 10, color: T.textFaint }}>profile.json</span>
                  <span style={{ fontSize: 9, color: signal }}>● saved</span>
                </div>
                <div style={{ aspectRatio: "4/3", background: T.bgAlt, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                  <img src="/profile.png" alt="Surya Prakash" style={{ width: "100%", height: "100%", objectFit: "contain" }}
                    onError={e => { const t = e.target as HTMLImageElement; t.style.display = "none"; (t.nextSibling as HTMLElement).style.display = "flex"; }} />
                  <div style={{ display: "none", width: "100%", height: "100%", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ fontSize: 56, fontWeight: 700, color: signal, fontFamily: "'Space Grotesk',sans-serif" }}>S</span>
                  </div>
                </div>
                ...
                <div style={{ borderTop: `1px solid ${T.line}`, padding: "10px 16px", display: "flex", gap: 8, flexWrap: "wrap", fontFamily: "'JetBrains Mono',monospace" }}>
                  {["Python","React","Node.js"].map(t => (
                    <span key={t} style={{ fontSize: 10, fontWeight: 600, padding: "3px 8px", background: T.bgAlt, border: `1px solid ${T.line}`, color: T.fn }}>{t}</span>
                  ))}
                </div>
              </div>
            </div>
      ...
      <section id="about" style={{ ...SP, background: T.bgAlt }}>
        <div style={C}>
          <STitle tag="01 — about.tsx" title="AboutMe" sub="" />
          <div className="about-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 44, alignItems: "start" }}>
            <Reveal dir="left">
              <div style={{ padding: 28, border: `1px solid ${T.line}`, background: T.bgCard }}>
                <p style={{ fontSize: 14, color: T.ink, lineHeight: 1.85, marginBottom: 16 }}>{DATA.bio}</p>
                <p style={{ fontSize: 14, color: T.textMuted, lineHeight: 1.85 }}>{DATA.bio2}</p>
                <div style={{ marginTop: 24, paddingTop: 20, borderTop: `1px solid ${T.line}`, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  <Field label="location">{DATA.location}</Field>
                  <Field label="university">PES University</Field>
                  <Field label="email">{DATA.email}</Field>
                  <Field label="phone">{DATA.phone}</Field>
            ...
            <Reveal dir="right">
              ...
              <div style={{ marginTop: 18, padding: "20px 24px", border: `1px solid ${T.line}`, background: T.bgCard }}>
                <div style={{ fontSize: 10, color: T.textFaint, marginBottom: 16, fontFamily: "'JetBrains Mono',monospace" }}>// git log --oneline</div>
                {[
                  { year: "2023", label: "Joined PES University" },
                  { year: "2024", label: "SMCC Head — Equinox Club" },
                  { year: "2025", label: "Research Intern at CCNCS" },
                  ...
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10, fontFamily: "'JetBrains Mono',monospace" }}>
                    <span style={{ color: T.num, fontSize: 11 }}>●</span>
                    <span style={{ fontSize: 10, color: item.year === "Now" ? signal : T.textFaint, flexShrink: 0, width: 32 }}>{item.year}</span>
                    <span style={{ fontSize: 12.5, color: T.ink, fontFamily: "'Inter',sans-serif" }}>{item.label}</span>
                  </div>
                ))}
              </div>
            </Reveal>
      ...
      <GutterDivider count={2} />

      {/* ── SKILLS ── */}
      <section id="skills" style={{ ...SP }}>
        <div style={C}>
          <STitle tag="02 — skills.tsx" title="TechStack" sub="Languages, frameworks, and tools I use to ship end-to-end — one module file per category." />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 16 }}>
            {SKILL_CATEGORIES.map((cat, i) => <SkillCategoryPanel key={cat.label} cat={cat} index={i} />)}
          </div>
        </div>
      ...
      <section id="experience" style={{ ...SP, background: T.bgAlt }}>
        <div style={{ ...C, maxWidth: 840 }}>
          <STitle tag="03 — experience.tsx" title="CallStack" sub="" />
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {DATA.experiences.map((exp, i) => <ExpCard key={i} exp={exp} index={i} />)}
          </div>
        </div>
        ...
        <div style={C}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16, marginBottom: 44 }}>
            <Reveal>
              <div>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, color: T.textFaint, marginBottom: 12 }}>
                  <span>{"/* "}</span><span style={{ color: signal }}>04 — projects.tsx</span><span>{" */"}</span>
                </div>
                <h2 style={{ fontSize: "clamp(26px,3.8vw,40px)", fontWeight: 700, color: T.ink, letterSpacing: "-0.01em", fontFamily: "'Space Grotesk',sans-serif" }}>
                  <span style={{ color: T.kw }}>class</span> FeaturedProjects <span style={{ color: T.textFaint }}>{"{"}</span>
                </h2>
              </div>
            </Reveal>
            <Reveal delay={0.1}><GithubLink /></Reveal>
      ...
      <section id="contact" style={{ ...SP, paddingBottom: 130, background: T.bgAlt }}>
        <div style={{ ...C, maxWidth: 940 }}>
          <STitle tag="05 — contact.tsx" title="GetInTouch" sub="Open to internships, research collabs, and freelance. DM anytime." />
          <div className="contact-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
            <div>
              <Reveal>
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 22 }}>
            ...
            <Reveal delay={0.2}>
              <div style={{ border: `1px solid ${T.lineStrong}` }}>
                <div style={{ padding: "10px 16px", background: T.bgAlt, borderBottom: `1px solid ${T.line}`, fontFamily: "'JetBrains Mono',monospace" }}>
                  <span style={{ fontSize: 10, color: T.textFaint }}>availability.config.json</span>
                </div>
                <div style={{ padding: 26, background: T.bgPanel }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 22 }}>
                    <Field label="hiring" accent>Yes</Field>
                    <Field label="responseTime">&lt; 24 hours</Field>
                  </div>
                  <Field label="modes">Internship · Collab · Freelance · OSS</Field>
                  <p style={{ fontSize: 12.5, color: T.textMuted, lineHeight: 1.8, margin: "16px 0 24px" }}>
                    Open to internships, research collaborations, freelance projects, and open-source work. Let's build something impactful together.
      ...
      {/* ── FOOTER ── */}
      <footer style={{ borderTop: `1px solid ${T.line}`, background: T.bg, padding: "24px 0 40px", position: "relative", zIndex: 2 }}>
        <div style={{ ...C, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, fontFamily: "'JetBrains Mono',monospace" }}>
          <p style={{ fontSize: 10.5, color: T.textFaint }}>{"// © "}{new Date().getFullYear()}{" "}<span style={{ color: T.textMuted }}>Gonella Siva Sai Surya Prakash</span>. All rights reserved.</p>
          <p style={{ fontSize: 10.5, color: T.textFaint }}>React · TypeScript · CSS-in-JS · 0 errors</p>
        </div>
      </footer>

      <BackToTop show={showBtt} />
      <StatusBar section={active} />
    </div>
  );
}
